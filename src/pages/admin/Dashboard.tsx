...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await ApiClient.get<{ statistics: DashboardStats, recentOrders: RecentOrder[] }>('/dashboard/stats');
  ...
  return (
    <div>
      <Helmet>
        <title>{t('admin.overview.title')} | {t('admin.dashboard')}</title>
        ...
        {cards.map((card, i) => {
          ...
          return (
            <div key={i} className={`rounded-2xl p-6 flex flex-col transition-all duration-300 hover:shadow-md hover:-translate-y-1 relative overflow-hidden group ${card.bg} border border-transparent`}>
              ...
              <div className="absolute -right-6 -top-6 w-24 h-24 bg-white/40 dark:bg-black/10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500" aria-hidden="true"></div>
              ...
              <Link 
                to={card.link} 
                className="mt-auto text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1 transition-colors z-10 w-fit"
                aria-label={`${t('admin.overview.details')} ${card.title}`}
              >
                {t('admin.overview.details')} <ArrowUpRight className="w-4 h-4" aria-hidden="true" />
              </Link>
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 space-y-3">
            <Link to="/admin/projects" className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group">
              <div className="bg-amber-100 dark:bg-amber-900/40 text-amber-600 p-2 rounded-lg group-hover:scale-110 transition-transform"><Briefcase className="w-5 h-5" aria-hidden="true" /></div>
            ...
            <Link to="/admin/orders" className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group">
              <div className="bg-blue-100 dark:bg-blue-900/40 text-blue-600 p-2 rounded-lg group-hover:scale-110 transition-transform"><Clock className="w-5 h-5" aria-hidden="true" /></div>
            ...
            <Link to="/admin/services" className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group">
              <div className="bg-purple-100 dark:bg-purple-900/40 text-purple-600 p-2 rounded-lg group-hover:scale-110 transition-transform"><Wrench className="w-5 h-5" aria-hidden="true" /></div>
        ...
        <div className="lg:col-span-2">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link to="/admin/orders" className="text-sm text-amber-600 hover:text-amber-700 font-semibold" aria-label={t('admin.overview.details')}>{t('admin.overview.details')}</Link>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            {recentOrders.length === 0 ? (
              <EmptyState title="لا توجد أنشطة حديثة" variant="no-data" size="sm" withBackground={false} />
            ) : (
                  ...
                  <div key={i} className="p-4 sm:p-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-gray-900 dark:text-white truncate">
                        طلب {order.service?.title || 'خدمة غير محددة'} من <span className="text-amber-600">{order.customer_name}</span>
                      </p>

...
import { useSettings, useAuth, useTheme } from '../contexts';
...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex transition-colors duration-200">
      ...
      {/* Sidebar */}
      <aside 
        aria-label={t('admin.nav.sidebar', 'القائمة الجانبية')}
      ...
      >
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-800">
          <div className="flex items-center gap-3 text-amber-500 font-bold text-xl">
            ...
            <span>{settings?.siteName || t('home.title')}</span>
          </div>
          <button 
        ...
        <div className="p-6 border-b border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-sm">{user?.name || t('admin.role.admin', 'مدير النظام')}</p>
        ...
        <div className="p-4 border-t border-gray-800">
          ...
          >
            {isLoggingOut ? <Loader2 className="w-5 h-5 animate-spin" aria-hidden="true" /> : <LogOut className="w-5 h-5" aria-hidden="true" />}
            <span>{isLoggingOut ? t('admin.loggingOut', 'جاري الخروج...') : t('admin.logout')}</span>
          </button>
        </div>
      </aside>
        ...
        <header className="h-20 bg-white dark:bg-gray-800 shadow-sm border-b border-gray-100 dark:border-gray-700 flex items-center justify-between px-6 lg:px-10 z-10 shrink-0 transition-colors duration-200">
          <div className="flex items-center gap-4">
            <button 
              ...
              aria-label={t('admin.a11y.openMenu', 'فتح القائمة')}
            >
              <Menu className="w-6 h-6" aria-hidden="true" />
            </button>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white hidden sm:block">
              {t('admin.dashboard', 'لوحة الإدارة')} - {settings?.siteName || t('home.title')}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <button 
              ...
              aria-label={t('admin.a11y.toggleLang', 'تغيير اللغة')}
            >
            ...
            <button 
              onClick={toggleTheme} 
              className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" 
              aria-label={t('admin.a11y.toggleTheme', 'تبديل المظهر')}
            >
              {theme === 'dark' ? <Sun className="w-5 h-5 text-amber-500" aria-hidden="true" /> : <Moon className="w-5 h-5" aria-hidden="true" />}
            </button>
            ...
            <Link 
              to="/" 
              target="_blank" 
              className="text-sm font-semibold text-amber-600 hover:text-amber-700 bg-amber-50 px-4 py-2 rounded-full hidden sm:block"
              aria-label={t('admin.header.viewSite')}
            >
